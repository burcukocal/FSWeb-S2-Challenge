...
function cumlelereDonustur(dizi, ayrac=",") {
  ...
  var yeniDizi = [];
  const cumleYap = dizi.map(cumle => cumle.join(ayrac));
  for(let i=0; i<cumleYap.length; i++){
    yeniDizi.push(cumleYap[i])
  }   
...
function paragrafOlustur(cumleler, cumleKur, cumlelereDonustur) {
  ...
  yeniDizi = cumleKur(yeniDizi[1],yeniDizi[3],yeniDizi[5],yeniDizi[7],yeniDizi[9]) 
...
function emojileriDonustur(mesaj, nesne) {
  ...
  for(let key in nesne){
      mesaj = mesaj.replaceAll(key.toUpperCase(),nesne[key]);
      mesaj = mesaj.replaceAll(key.toLowerCase(),nesne[key]);
    } 
